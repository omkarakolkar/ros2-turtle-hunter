...
class BatteryNode(Node): 
    def __init__(self):
        ...
        self.batery_timer_ = self.create_timer(0.1, self.check_battery_state)
        self.get_logger().info("Battery node starts")
    ...
    def call_set_led_server(self, led_number, state):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for server to ...")
    ...
    def callback_call_set_led(self, future, led_number, state):
        try:
            ...
            self.get_logger().error("Servicce call failed %r" % (e,))
